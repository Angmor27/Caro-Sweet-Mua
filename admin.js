...
function switchView(view) {
  ...
  currentView = view;
  const titles = { hoy:'Hoy', citas:'Todas las citas', bloquear:'Bloquear horarios', resenas:'Reseñas', config:'Configuración' };
  document.getElementById('topbarTitle').textContent = titles[view] || view;
  if (view === 'hoy')      loadHoyView();
  if (view === 'citas')    loadCitasView();
  if (view === 'bloquear') loadBloquearView();
  if (view === 'resenas')  loadResenasAdminView();
...
async function loadTelefono() {
  ...
  try {
    ...
    if (doc.exists && doc.data().telefono) {
      ...
      document.getElementById('telefonoInput').value = '528134559972';
    }
...
function abrirFormResena() {
  document.getElementById('formResenaWrap').classList.remove('hidden');
}
function cerrarFormResena() {
  document.getElementById('formResena').reset();
  document.getElementById('formResenaWrap').classList.add('hidden');
}

document.getElementById('formResena').addEventListener('submit', async e => {
  e.preventDefault();
  if (!db) { showToast('Firebase no configurado'); return; }
  const btn = e.target.querySelector('[type=submit]');
  btn.disabled = true; btn.textContent = 'Guardando...';
  try {
    await db.collection(RESENAS_COL).add({
      nombre:    document.getElementById('rNombre').value.trim(),
      texto:     document.getElementById('rTexto').value.trim(),
      estrellas: Number(document.getElementById('rEstrellas').value),
      visible:   false,
      fecha:     firebase.firestore.FieldValue.serverTimestamp()
    });
    showToast('✅ Reseña guardada (oculta por defecto)');
    cerrarFormResena();
    loadResenasAdminView();
  } catch(err) {
    showToast('Error al guardar: ' + err.message);
  } finally {
    btn.disabled = false; btn.textContent = 'Guardar reseña';
  }
});

async function loadResenasAdminView() {
  const grid  = document.getElementById('resenasAdminGrid');
  const empty = document.getElementById('resenasAdminEmpty');
  grid.innerHTML = '<p style="color:var(--muted)">Cargando...</p>';

  if (!db) {
    grid.innerHTML = '<p style="color:var(--muted)">Firebase no configurado.</p>';
    return;
  }

  try {
    const snap = await db.collection(RESENAS_COL).orderBy('fecha', 'desc').get();
    if (snap.empty) {
      grid.innerHTML = '<p style="color:var(--muted)">No hay reseñas todavía.</p>';
      return;
    }
    grid.innerHTML = '';
    snap.forEach(doc => {
      const d   = doc.data();
      const stars = '★'.repeat(d.estrellas || 5);
      const fecha = d.fecha?.toDate
        ? d.fecha.toDate().toLocaleDateString('es-MX', { year:'numeric', month:'long', day:'numeric' })
        : '—';

      const row = document.createElement('div');
      row.className = 'resena-admin-row';
      row.style.cssText = 'background:#fff;border:1px solid var(--border);border-radius:12px;padding:16px 20px;display:flex;align-items:flex-start;gap:16px;flex-wrap:wrap';
      row.innerHTML = `
        <div style="flex:1;min-width:200px">
          <div style="font-weight:600;font-size:.9rem">${d.nombre}</div>
          <div style="color:#c9a96e;font-size:.9rem;margin:2px 0">${stars}</div>
          <div style="font-size:.88rem;color:#444;margin-top:4px;font-style:italic">"${d.texto}"</div>
          <div style="font-size:.75rem;color:var(--muted);margin-top:6px">${fecha}</div>
        </div>
        <div style="display:flex;flex-direction:column;gap:8px;align-items:flex-end">
          <label class="resena-toggle" title="${d.visible ? 'Ocultar en el sitio' : 'Mostrar en el sitio'}">
            <input type="checkbox" ${d.visible ? 'checked' : ''} onchange="toggleResena('${doc.id}', this.checked)" />
            <span class="resena-toggle__track"></span>
            <span style="font-size:.78rem;color:var(--muted)">${d.visible ? 'Visible' : 'Oculta'}</span>
          </label>
          <button onclick="eliminarResena('${doc.id}', this)" style="background:none;border:none;color:#c0392b;cursor:pointer;font-size:.8rem;padding:4px 0">🗑 Eliminar</button>
        </div>`;
...
async function toggleResena(id, visible) {
  try {
    await db.collection(RESENAS_COL).doc(id).update({ visible });
    showToast(visible ? '✅ Reseña publicada en el sitio' : '🙈 Reseña ocultada del sitio');
    loadResenasAdminView();
  } catch(err) { showToast('Error: ' + err.message); }
}

async function eliminarResena(id, btn) {
  if (!confirm('¿Eliminar esta reseña permanentemente?')) return;
  btn.disabled = true;
  try {
    await db.collection(RESENAS_COL).doc(id).delete();
