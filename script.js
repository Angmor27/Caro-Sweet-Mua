...
(function loadResenas() {
  let db = null;
  try {
    if (typeof firebase !== 'undefined' && FIREBASE_CONFIG.apiKey !== 'PEGA_AQUI') {
  ...
  if (!db) { empty.style.display = 'block'; return; }
  ...
  db.collection('resenas')
    ...
    .then(snap => {
      ...
      let count = 0;
      snap.forEach(doc => {
        if (!doc.data().visible) return;
        const d = doc.data();
        const stars = '★'.repeat(Math.min(5, d.estrellas || 5));
        const inicial = (d.nombre || '?')[0].toUpperCase();
        const fecha = d.fecha?.toDate
          ? d.fecha.toDate().toLocaleDateString('es-MX', { year:'numeric', month:'long' })
          : '';
        const card = document.createElement('div');
        card.className = 'resena-card';
        card.innerHTML = `
          <div class="resena-card__stars">${stars}</div>
          <p class="resena-card__texto">"${d.texto}"</p>
          <div class="resena-card__autor">
            <div class="resena-card__avatar">${inicial}</div>
            <div>
              <div class="resena-card__nombre">${d.nombre}</div>
    ...
    .catch(err => { console.error('Reseñas error:', err); empty.style.display = 'block'; });
})();
...
document.querySelectorAll('.star-picker span').forEach(star => {
  star.addEventListener('click', () => {
    ...
    document.getElementById('rpEstrellas').value = val;
    document.querySelectorAll('.star-picker span').forEach(s => {
      s.classList.toggle('active', Number(s.dataset.val) <= val);
    });
  });
});
// Inicializar en 5 estrellas
document.querySelectorAll('.star-picker span').forEach(s => s.classList.add('active'));

// ── Formulario público de reseña ────────────────────────────
const resenaPublicaForm = document.getElementById('resenaPublicaForm');
if (resenaPublicaForm) {
  resenaPublicaForm.addEventListener('submit', async e => {
    e.preventDefault();
    const btn     = resenaPublicaForm.querySelector('[type=submit]');
    const success = document.getElementById('rpSuccess');
    btn.disabled = true; btn.textContent = 'Enviando...';
    const nombre    = document.getElementById('rpNombre').value.trim();
    const texto     = document.getElementById('rpTexto').value.trim();
    const estrellas = Number(document.getElementById('rpEstrellas').value);
    ...
    try {
      ...
      resenaPublicaForm.reset();
      document.querySelectorAll('.star-picker span').forEach(s => s.classList.add('active'));
      document.getElementById('rpEstrellas').value = 5;
...
const GALLERY = [
  ...
  // ── PEINADOS SUELTOS ───────────────────────────────────
  { cat:'peinados', type:'img', src:'img/Peinado suelto/cabello.jpg' },
  // ── PAQUETE ────────────────────────────────────────────
